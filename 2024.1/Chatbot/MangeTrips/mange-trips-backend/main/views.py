...
class ChatBotAPIView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return JsonResponse(status=500, data={'content': 'User not found'})

        conver = None

        if conversationId is None:
            newHistory = ConversationHistory(user=user)
            newHistory.save()
            ...
            try:
                conver = ConversationHistory.objects.get(pk=conversationId)
                
            except:
                return JsonResponse(status=500, data={'content': 'Conversation not found'})
            
        newQuestion = Conversation(type='Q', message=question, history=conver)
        newQuestion.save()

        answer = chat.get_response(question)

        newAnswer = Conversation(type='A', message=answer.message, history=conversationId)
        newAnswer.save()

        serializer = ConversationSerializer(newAnswer, many=False)
